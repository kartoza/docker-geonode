...
LABEL org=kartoza
LABEL maintainer.name="Rizky Maulana Nugraha"
LABEL maintainer.email=lana.pcfre@gmail.com

# Characteristics of this base image variants:
# - contain postgresql client dependencies
# - contain GDAL dependencies
# - does not contain cron (should be a microservice)
# - does not contain memcached (should be a microservice)
# - does not contain development packages
...
COPY ./geonode/wait-for-databases.sh /usr/bin/wait-for-databases
RUN chmod +x /usr/bin/wait-for-databases
RUN chmod +x /usr/src/geonode/tasks.py \
    && chmod +x /usr/src/geonode/entrypoint.sh

# Production mode entrypoint
COPY ./variants/prod/prod-entrypoint.sh /entrypoint.sh
...
CMD ["uwsgi", "--ini", "/usr/src/geonode/uwsgi.ini"]
...
COPY ./variants/prod/dev-entrypoint.sh /entrypoint.sh
RUN chmod +x /entrypoint.sh

EXPOSE 8001

ENTRYPOINT /entrypoint.sh
CMD ["/usr/local/bin/python","manage.py","runserver","0.0.0.0:8001"]
